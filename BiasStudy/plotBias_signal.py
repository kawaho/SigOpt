...
for cat in cats:
  outfile = open(cat+'_signalBiasStudy.txt','a', buffering=0) 
  ...
  for sigtype in ['cb', 'gaus']:
    ...
    outfile.write("Plot:" + cat + " " + sigtype + " " + altsigtype +" "+ str(round(h.GetMean()*100,3)) + " " + str(round(h.GetStdDev(),3)) + "\n")
    outfile.write("Fit:" + cat + " " + sigtype + " " + altsigtype +" "+ str(round(h_mean*100,3)) + " " + str(round(h_sigma,3)) + "\n")
    file_.Close()
  outfile.close()
