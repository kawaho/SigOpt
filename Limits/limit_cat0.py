...
gROOT.SetBatch(True)
catnames = ["ggcat0+vbfcat0","vbfcat0","ggcat0"]
...
for c in catnames:
  ...
  for k in range(nentries):
    ...
    limits[-1].append(limitTree.limit*10)
print limits
print "==============================="
print "         SUMMARY TABLE         "
print "==============================="
...
for i in range(ncats):
  if i == 0:
    channels.append("#splitline{ggcat0 +}{#splitline{vbfcat0}{#scale[0.8]{(%1.2f #upoint 10^{-3})}}}"%(limits[i][2]))
  else:
    channels.append("#splitline{%s}{#scale[0.8]{(%1.2f #upoint 10^{-3})}}"%(catnames[i], limits[i][2]))
print channels
PLOTLIMIT.cd()

h = TH2F("h", "test", 1, 0, 10, ncats, 0, ncats)
for ch in channels:
  h.Fill(1, ch, 0)
h.SetXTitle("95% CL limit on #bf{#it{#Beta}}(H#rightarrow e#mu), 10^{-3}")
...
PLOTLIMIT.SaveAs("Comb_Limits_cat0.png")
