...
for whichcat in ['GGcat0', 'GGcat1', 'GGcat2', 'VBFcat']:
  ...
  for whichcat_deep in ['GG', 'VBF']:
    ...
    #Loop through quantiles and turn tree into RooDataSet
      ...
      for sys in leptonUnc:
        for UpDown in ['Up', 'Down']:
          datasets.append(r.RooDataSet("%s_%s_range%i"%(sys,UpDown,i+1), "%s_%s_range%i"%(sys,UpDown,i+1), tree, r.RooArgSet(mass_sys['CMS_emu_Mass_%s_%s'%(sys,UpDown)],mva_sys_dummy['mva_%s_%s'%(sys,UpDown)],weight_dummy), "(mva_%s_%s<%f) && (mva_%s_%s>=%f)"%(sys,UpDown,quantiles[i+1],sys,UpDown,quantiles[i]), "weight"))
    #output all RooDataSet into workspace
    w = r.RooWorkspace("CMS_emu_workspace", "CMS_emu_workspace")
    for dataset in datasets:
      getattr(w, 'import')(dataset)
