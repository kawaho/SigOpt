...
for vbf_gg in ['gg', 'vbf']:
  ...
  for i in range(len(range_[vbf_gg])-1):
    ...
    if args.limit:
      numberOfBkg[catnames[-1]] = fit(inWS, file_gg_full[vbf_gg].Get("CMS_emu_workspace"), file_vbf_full[vbf_gg].Get("CMS_emu_workspace"), 'exp1', [rangl, rangh], catnames[-1], makePlot=True, saveData=False, sys_=True, bin_=False)[1]
    else:
