...
for cat in cats:
  ...
  for prod in ['ggH','qqH']:
    ...
    mean_spline.append(r.RooSpline1D('%s_%s_mean_cbe_nom'%(cat,prod),'%s_%s_mean_cbe_nom'%(cat,prod),mh,len(masspts),np.array([float(mp) for mp in masspts]),np.array(params['dm'])))
