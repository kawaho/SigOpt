...
t.GetEntry(0)
for poi in opt.POIs.split(","): poi_bf[poi] = getattr(t,poi)#w.var(poi).getVal()
...
if opt.batch == 'IC':
  ...
  for itoy in range(0,opt.nToys):
    ...
    fsub.write("#!/bin/bash\n\n")
    fsub.write("cd %s/src/HiggsAnalysis/CombinedLimit/SigOpt/S+Btoys/SplusBModels%s/toys\n\n"%(os.environ['CMSSW_BASE'],opt.ext))
  ...
  fcondor.write("output                = %s/src/HiggsAnalysis/CombinedLimit/SigOpt/S+Btoys/SplusBModels%s/toys/jobs/sub_toys.$(ClusterId).$(ProcId).out\n"%(os.environ['CMSSW_BASE'],opt.ext))
  fcondor.write("error                 = %s/src/HiggsAnalysis/CombinedLimit/SigOpt/S+Btoys/SplusBModels%s/toys/jobs/sub_toys.$(ClusterId).$(ProcId).err\n"%(os.environ['CMSSW_BASE'],opt.ext))
  fcondor.write("log                   = %s/src/HiggsAnalysis/CombinedLimit/SigOpt/S+Btoys/SplusBModels%s/toys/jobs/sub_toys.$(ClusterId).log\n\n"%(os.environ['CMSSW_BASE'],opt.ext))
